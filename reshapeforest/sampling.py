...
class VSSampling(Sampling):
    ...
    def __init__(self, num_samples, lower_bound=256, upper_bound=1024, vs_type='EXP', bootstrap=False, bagging=None):
        ...
        if self._bootstrap:
            for i in range(self._num_samples):
                ...
                if self._vs_type == 'FIX':
                    ...
                    indices = np.random.randint(0, data_size,
                                                int(round(np.random.uniform(self._lower_bound, self._upper_bound))))
                elif self._vs_type == 'EXP':
                    indices = np.random.randint(0, data_size, int(round(
                        np.power(2.0, np.random.uniform(np.log2(self._lower_bound), np.log2(self._upper_bound))))))
            ...
            for i in range(self._num_samples):
                ...
                if self._vs_type == 'UNI':
                    ...
                    sample_size = min(data_size, int(round(
                        np.power(2.0, np.random.uniform(np.log2(self._lower_bound), np.log2(self._upper_bound))))))
